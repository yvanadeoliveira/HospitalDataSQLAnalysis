...
-- Calculating the total costs by encounter class

SELECT ENCOUNTERCLASS, ROUND(AVG(TOTAL_CLAIM_COST), 2) AS avg_cost
FROM encounters
GROUP BY ENCOUNTERCLASS;
...
-- Identifying the most important insurance partners

SELECT p.name, COUNT(DISTINCT e.patient) AS covered_patients
FROM payers p
JOIN encounters e ON p.id = e.payer
GROUP BY p.name
ORDER BY covered_patients DESC
LIMIT 5;

-- Identifying the financial impact of readmissions

WITH cte AS (
  SELECT patient, start, stop, total_claim_cost,
         LEAD(start) OVER (PARTITION BY patient ORDER BY start) AS next_start
  FROM encounters
)
SELECT COUNT(*) AS readmissions, 
       ROUND(AVG(total_claim_cost), 2) AS avg_readmission_cost
FROM cte
